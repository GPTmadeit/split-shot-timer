...
/**
 * Stock Android theming.
 *
 * Material You first: on Android 12+ the palette is derived from the user's
 * wallpaper, which is what makes an app look like it belongs on the device
 * rather than imposing its own brand on someone's home screen. The hand-built
 * schemes below are the fallback for Android 10 and 11.
 *
 * The instrument colours (HiViz, Brass, Good, Bad) stay fixed regardless. A
 * wallpaper-derived pair could easily be low-contrast, and split times have to
 * stay readable.
 */
@OptIn(ExperimentalMaterial3ExpressiveApi::class)
@Composable
fun SplitTheme(dark: Boolean, dynamic: Boolean = true, content: @Composable () -> Unit) {
